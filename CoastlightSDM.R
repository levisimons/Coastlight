...
if(species=="Grunion"){
  dev.off()
  png(paste("RandomForestResponse",species,".png",sep=""),width=1000,height=1000,res=300)
  #p1 <- partial(rf1,pred.var = "DEM2mRasterAligned",train=envtrain[,-c(1)]) %>% plotPartial(smooth=TRUE,lwd=2,ylab="predicted value",xlab="Elevation (m)")
  p2 <- partial(rf1,pred.var = "DistanceToFreshwaterClipAligned",train=envtrain[,-c(1)]) %>% plotPartial(smooth=TRUE,lwd=2,ylab="predicted value",xlab="Distance to freshwater (m)")
  #p3 <- partial(rf1,pred.var = "DistanceToSaltwaterClipAligned",train=envtrain[,-c(1)]) %>% plotPartial(smooth=TRUE,lwd=2,ylab="predicted value",xlab="Distance to saltwater (m)")
  p4 <- partial(rf1,pred.var = "LogSIAligned",train=envtrain[,-c(1)]) %>% plotPartial(smooth=TRUE,lwd=2,ylab="predicted value",xlab="Log(SI) log(mlx)")
  p5 <- partial(rf1,pred.var = "Slope2mRasterAligned",train=envtrain[,-c(1)]) %>% plotPartial(smooth=TRUE,lwd=2,ylab="predicted value",xlab="Slope (%)")
  ...
  p7 <- partial(rf1,pred.var = "SoCalBeachWidthAligned",train=envtrain[,-c(1)]) %>% plotPartial(smooth=TRUE,lwd=2,ylab="predicted value",xlab="Beach width (m)")
  p8 <- partial(rf1,pred.var = "SVF2mAligned",train=envtrain[,-c(1)]) %>% plotPartial(smooth=TRUE,lwd=2,ylab="predicted value",xlab="SVF")
  grid.arrange(p2,p4,p5,p6,p7,p8,ncol=3)
  dev.off()
}
if(species=="Plover"){
  dev.off()
  png(paste("RandomForestResponse",species,".png",sep=""),width=1000,height=1000,res=300)
  p1 <- partial(rf1,pred.var = "DEM2mRasterAligned",train=envtrain[,-c(1)]) %>% plotPartial(smooth=TRUE,lwd=2,ylab="predicted value",xlab="Elevation (m)")
  p2 <- partial(rf1,pred.var = "DistanceToFreshwaterClipAligned",train=envtrain[,-c(1)]) %>% plotPartial(smooth=TRUE,lwd=2,ylab="predicted value",xlab="Distance to\nfreshwater (m)")
  p3 <- partial(rf1,pred.var = "DistanceToSaltwaterClipAligned",train=envtrain[,-c(1)]) %>% plotPartial(smooth=TRUE,lwd=2,ylab="predicted value",xlab="Distance to\nsaltwater (m)")
  p4 <- partial(rf1,pred.var = "LogSIAligned",train=envtrain[,-c(1)]) %>% plotPartial(smooth=TRUE,lwd=2,ylab="predicted value",xlab="Log(SI) log(mlx)")
  p5 <- partial(rf1,pred.var = "Slope2mRasterAligned",train=envtrain[,-c(1)]) %>% plotPartial(smooth=TRUE,lwd=2,ylab="predicted value",xlab="Slope (%)")
  p6 <- partial(rf1,pred.var = "SoCalBeachTypeAligned",train=envtrain[,-c(1)]) %>% plotPartial(smooth=TRUE,lwd=2,ylab="predicted value",xlab="Beach category")
  p7 <- partial(rf1,pred.var = "SoCalBeachWidthAligned",train=envtrain[,-c(1)]) %>% plotPartial(smooth=TRUE,lwd=2,ylab="predicted value",xlab="Beach width (m)")
...
if(species=="Grunion"){
  dev.off()
  png(paste("MaxentResponsePlot",species,".png",sep=""),width=1000,height=1000,res=300)
  #p1 <- response(xm,var="DEM2mRasterAligned")
  p2 <- response(xm,var="DistanceToFreshwaterClipAligned")
  #p3 <- response(xm,var="DistanceToSaltwaterClipAligned")
  p4 <- response(xm,var="LogSIAligned")
  p5 <- response(xm,var="Slope2mRasterAligned")
  p6 <- response(xm,var="SoCalBeachTypeAligned")
  p7 <- response(xm,var="SoCalBeachWidthAligned")
  p8 <- response(xm,var="SVF2mAligned")
  par(mfrow=c(2,3))
